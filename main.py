...
def train(
    ...
    for epoch_no in range(config["epochs"]):
        ...
        for batch_no, train_batch in enumerate(train_loader):
            ...
            loss = alpha * loss_noise + beta * loss_cons  
            loss.backward()
            optimizer.step()
            avg_loss += loss.item()
...
def evaluate(
    ...
    with torch.no_grad():
        ...
        with tqdm(test_loader, mininterval=5.0, maxinterval=50.0) as it:
            ...
            print("mae = {:.3f}, rmse = {:.3f}".format(mae, rmse))
...
def main(args):
    ...
    if args.scratch:
        ...
        model.load_state_dict(
            torch.load(saving_path + "/model_{}.pth".format(current_time), map_location={'cuda:2': 'cuda:0'})
        )
    else:
        print("load model from", args.checkpoint_path)
        model.load_state_dict(torch.load(args.checkpoint_path), map_location={'cuda:2': 'cuda:0'})
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--checkpoint_path",
        type=str,
        default="../saved_models/ETT/block/0.2/model.pth",
    )
    ...
    parser.add_argument("--scratch", action="store_true", help="test or scratch",default=True)
